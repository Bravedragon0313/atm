...
git clone https://github.com/atm_base/atm_base-led
git clone https://github.com/atm_base/atm_base-machine -b $TAG
...
cd .. && rm -rf atm_base-led && cd atm_base-machine
...
# final machine path
cd .. && sudo mv atm_base-machine /opt
...
sudo cp -r /opt/atm_base-machine/hardware/system/upboard/$MACHINE/supervisor/conf.d/ /etc/supervisor/
sudo sed -i 's/user=machine/user=ubilinux/g' /etc/supervisor/conf.d/atm_base-browser.conf

# Udev config files
sudo cp -r /opt/atm_base-machine/hardware/system/upboard/$MACHINE/udev/* /etc/udev/rules.d/
