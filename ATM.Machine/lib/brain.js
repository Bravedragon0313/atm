...
const BILL_ACCEPTING_STATES = ['billInserted', 'billRead', 'acceptingBills', 'acceptingFirstBill', 'maintenance']
const NON_TX_STATES = ['networkDown', 'connecting', 'wifiConnected', 'pairing', 'initializing', 'booting']
...
Brain.prototype.determinePlatform = function determinePlatform() {
    ...
    if (fs.existsSync('/etc/init/atm_base-machine.conf')) return 'AAEON'
    return 'SSUBOARD'
}
...
Brain.prototype.run = function run() {
    console.log('ATM Machine software initialized.')
    const self = this
    this._init()
    boardManager.run()
...
Brain.prototype._connectedBrowser = function _connectedBrowser() {
    ...
    const atmModel = this.rootConfig.atmModel || 'atm_base'
...
Brain.prototype.hasNewScanBay = function hasNewScanBay() {
    const model = deviceConfig.atmModel
    return model === 'atm_base'
}
...
Brain.prototype.isStaticState = function isStaticState() {
    const staticStates = ['chooseCoin', 'idle', 'pendingIdle', 'dualIdle', 'networkDown', 'unpaired', 'maintenance', 'virgin', 'wifiList']
...
Brain.prototype._restartServices = function _restartServices(reason, restartBrowser) {
    ...
    const command = this.determinePlatform === 'AAEON'
        ? 'killall chromium-browser' : 'supervisorctl restart atm_base-browser'
...
function pickBoardManager() {
    if (deviceConfig.atmModel === 'atm_base' && !deviceConfig.brain.hasSsuboard) {
        return require('./upboard/atm_base/board-manager')
    }
