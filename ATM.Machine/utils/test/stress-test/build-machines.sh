...
if [ $# -eq 0 ]
  then
    echo "usage: ./build-machines [number_of_machines] /path/to/server/cert/atm_base_op_root_ca.pem" && exit 1
fi

case $1 in
    ''|*[!0-9]*) echo "usage: ./build-machines [number_of_machines] /path/to/server/cert/atm_base_op_root_ca.pem" && exit 1;;
esac

SERVER_CERT=$(perl -pe 's/\n/\\n/' < $2)
if [ -z "$SERVER_CERT" ]
  then
    echo "atm_base-op-root-ca.pem is empty" && exit 1
fi
...
EOL
  ...
  # Update db config
  sudo -u postgres psql -t -d atm_base-stress -c "select data from user_config where type='config' order by id desc limit 1" > config.json
  NEW_CONFIG=$(node ./save-config.js $NUMBER $DEVICE_ID)
  sudo -u postgres psql "atm_base-stress" << EOF
