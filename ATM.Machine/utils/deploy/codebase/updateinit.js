...
function installDeviceConfig (cb) {
  try {
    const currentDeviceConfigPath = `${applicationParentFolder}/ATM.machine/config/device_config.json`
...
async.series([
  ...
  async.apply(command, `cp -PR /tmp/extract/package/subpackage/atm_base-machine ${applicationParentFolder}`),
  async.apply(command, `cp -PR /tmp/extract/package/subpackage/hardware/${hardwareCode}/node_modules ${applicationParentFolder}/atm_base-machine/`),
